...
<nav class="z-[8] fixed h-[10vh] top-0 bg-dark dark:bg-light bg-opacity-90 w-full px-6 flex justify-between items-center">
    ...
    <div class="flex justify-center items-center">
        ...
        {:else}
            <a id="joinUsNav" class="menuButton h-9 w-auto bg-light dark:bg-dark hover:bg-dark px-4 font-display font-medium text-dark hover:text-light ease-out duration-200 uppercase rounded-xl border-2 border-light">Join us</a>
            <a id="signInNav" class="menuButton h-9 w-auto bg-dark dark:bg-light hover:bg-light px-4 font-display font-medium text-light hover:text-dark ease-out duration-200 uppercase rounded-xl border-2 border-light ml-4">Sign In</a>
        {/if}
