...
<nav class="z-[8] fixed h-[10vh] top-0 bg-light dark:bg-dark bg-opacity-90 w-full px-6 flex justify-between items-center">
    ...
    <div class="flex justify-center items-center">
        {#if SignedIn}
            <SlidingButton hasFill buttonText={"Go To Dashboard"} textSize={20} paddingX={16} paddingY={2} marginTop={0} marginRight={0}/>
        {:else}
        ...
        {/if}
            <button id="darkModeButton" class="menuButton flex justify-center items-center h-9 w-9 bg-dark px-0 font-display font-medium text-light  ease-out duration-200 uppercase rounded-xl border-2 border-light ml-4 cursor-none" on:click={toggleTheme}>
